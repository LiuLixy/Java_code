...
create table if not exists memo_group (
	...
	backgroup enum('WHITE', 'RED', 'BLUE', 'GREEN') default 'WHITE' comment '背景颜色',
...
-- 欢迎标签
create table memo_info (id int, group_id int, title varchar(32), content varchar(1024), create_time datetime not null)engine innodb; 
insert into memo_info (id, group_id, title, content, create_time) values (1,1,'欢迎使用','下面是使用手册', now());
